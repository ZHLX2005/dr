...
| `logs.sh` | `docker-compose logs -f --tail=100` |
| `shell.sh` | `docker-compose exec <service> sh` 或 `bash` |
| `backup.sh` | 备份 `./data/` 到 `./backup/backup-$(date +%Y%m%d).tar.gz` |
...
```bash
#!/bin/sh
# example/sh/backup.sh
SERVICE_NAME="$(basename "$(dirname "$(dirname "$0")")")"
SERVICE_DIR="$(dirname "$0")/../.."
BACKUP_DIR="$SERVICE_DIR/backup"
mkdir -p "$BACKUP_DIR"
tar czf "$BACKUP_DIR/backup-$(date +%Y%m%d_%H%M%S).tar.gz" -C "$SERVICE_DIR/$SERVICE_NAME" data/ logs/
